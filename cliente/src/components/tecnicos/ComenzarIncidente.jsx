...
function ComenzarIncidente() {
  ...
  const onSubmit = async () => {
    try {
      await axios.put(
        "http://localhost:3000/api/incidentes/comenzar/" + incidenteStore.id
      );
      toast.success("Incidente tomado");
    } catch (error) {
      console.error(error);
    ...
    <>
      ...
      >
        <article className="flex justify-between items-start">
          <h2 className="text-2xl">Comenzar incidente</h2>
          <button
            type="button"
            onClick={() => {
...
export default ComenzarIncidente;
